#include "CAnimate.h"
	 ...
	 void CAnimate::SetMoverState(bool b)
	{
	 ...
	 void CAnimate::SetMover(CMotion *mover)
	{
		mMover = mover;
		mMover->Welcome(mXPos, mYPos, W, H, mVelocity);
	}
	 ...
	 void CAnimate::OnAnimate()
	{
	 ...
	 void CAnimate::Move()
	{
		 // Tells the mover object to update the sprite's position.
		if (mMover != NULL)
		{
			mMover->Tick();
			SetPos(mMover->GetX(), mMover->GetY());
		}
	}
